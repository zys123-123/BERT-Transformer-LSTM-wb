...
embedding_pretrained = torch.tensor(np.load('./data/embedding_Tencent.npz')["embeddings"].astype('float32'))  # 预训练词向量
...
def load_dataset(path, pad_size, tokenizer, vocab):
    ...
    df['label'] = df['label'].astype(int)  # 将标签转换为整数
    for _, row in tqdm(df.iterrows(), total=df.shape[0]):
        label, content = row['label'], row['comment']
        words_line = []
