...
pretrain_config = {
    ...
    "batch_size": 64,  # 小批量训练
    "learning_rate": 5e-4,  # 更大的学习率，加速收敛
    "hidden_size": 128,  # LSTM 隐藏层维度
    "lstm_blocks": 2,  # LSTM并行处理数
    "num_layers": 2,  # LSTM 层数增加
    "transformer_layers": 2,  # Transformer 层数适当调整
...
# 微调阶段（目标数据集）的超参数配置
finetune_config = {
    "dropout": 0.3,  # 减小 Dropout
    "num_classes": 2,  # 分类任务类别数
    "num_epochs": 30,  # 微调的 epoch 数量
    "batch_size": 32,  # 批量大小适中
    "learning_rate": 2e-5,  # 小学习率
    "hidden_size": 128,  # LSTM 隐藏层维度
    "lstm_blocks": 2,  # LSTM并行处理数
    "num_layers": 2,  # LSTM 层数与预训练一致
    "transformer_layers": 2,  # Transformer 层数与预训练一致
    "transformer_heads": 8,  # 注意力头数与预训练一致
    "dim_feedforward": 512,  # 前馈层维度
}
...
def load_dataset(path, phase="finetune", test_size=0.4, dev_size=0.5, random_seed=42):
    ...
    try:
        ...
        if phase == "pretrain":
            ...
            dev, test = train_test_split(X_t, test_size=dev_size, random_state=random_seed + 1)
...
def get_data(data_path, phase="finetune", batch_sizes=None):
    ...
    if phase == "finetune":
        dataloaders["dev"] = create_dataloader(dev_data, batch_size=batch_sizes["dev"], shuffle=False,
                                               num_workers=num_workers)
        dataloaders["test"] = create_dataloader(test_data, batch_size=batch_sizes["test"], shuffle=False,
                                                num_workers=num_workers)
...
class Model(nn.Module):
    def __init__(self, hidden_size, num_classes, num_layers, lstm_blocks, dropout, transformer_layers,
                 transformer_heads, dim_feedforward):
        super(Model, self).__init__()
        # 加载预训练的 BERT 模型
        self.bert = BertModel.from_pretrained(local_bert_path)
        bert_output_dim = self.bert.config.hidden_size  # BERT 输出维度
        ...
        # Transformer 编码器层
        d_model = hidden_size * 2 * lstm_blocks  # 确保 Transformer 的 d_model 和 LSTM 输出对齐
        assert d_model % transformer_heads == 0, "d_model 必须能被 transformer_heads 整除"
    ...
    def forward(self, x, mask):
        ...
        for lstm in self.lstm_blocks:
            lstm_output, _ = lstm(bert_output.last_hidden_state)
            lstm_outputs.append(lstm_output)

        # 拼接多个 LSTM 块的输出
        concatenated_lstm_output = torch.cat(lstm_outputs, dim=2)  # 按特征维度拼接

        # Transformer 层
        transformer_output = self.transformer_encoder(concatenated_lstm_output)

        # 全连接层逐步处理
        x = torch.relu(self.fc1(transformer_output[:, -1, :]))
...
def train(model, dataloaders, config, save_path, log, phase="pretrain"):
    ...
    for epoch in range(num_epochs):
        ...
        if dataloaders.get('dev') is not None:
            ...
            # 保存最后一个 epoch 的模型
            if epoch == num_epochs - 1:  # 检查是否是最后一个 epoch
                torch.save(model.state_dict(), save_path)
                log.info(f"Pretrain model saved to {save_path} after last epoch.")
...
if __name__ == '__main__':
    ...
    pretrain_model = Model(
        ...
        lstm_blocks=pretrain_config['lstm_blocks'],
